...
public class FindQueryTest extends BaseTestApi {
    ...
    public void testOnlineSqlWriting() {
        ...
        jpOrm.tx(session -> {
            ...
            final String methodOneRendering = query.sqlQuery();
    ...
    public void testJoinWithInnerSelectQuery() {
        ...
        jpOrm.tx(session -> {
            ...
            SelectCommon query = session.find(Employee.class, "e1")
                    .innerJoin(session.find(People.class, "p").where("p.lastname = ?", "X"), "e2", "e1.name", "e2.firstname").where("mod(e1.id, 10) = ?", "Y");
