...
import com.jporm.mapper.clazz.BeanWithCustom.CustomGenerator;
...
public class ClassMapperGeneratorTest extends BaseTestApi {
    ...
    @Test
    public void testBeanWithCustomValuesGenerator() {
        final ClassMap<BeanWithCustom> classMapper = new ClassMapBuilderImpl<BeanWithCustom>(
                BeanWithCustom.class, new NullServiceCatalog()).generate();
        assertNotNull(classMapper);
        ClassFieldImpl<BeanWithCustom, Object> field = classMapper.getClassFieldByJavaName("custom");
        assertEquals(GeneratorType.CUSTOM, field.getGeneratorInfo().getGeneratorType());
        assertEquals(CustomGenerator.class, field.getGeneratorInfo().getValueGeneratorClass());
