...
public class RightOuterJoinElement<BEAN> extends AFromElement<BEAN> {
    ...
    public RightOuterJoinElement(final ClassDescriptor<BEAN> classDescriptor, final Class<?> joinClass, final Integer nameSolverClassId) {
        this(classDescriptor, joinClass, nameSolverClassId, "", ""); //$NON-NLS-1$ //$NON-NLS-2$
        onClause = false;
    }

    public RightOuterJoinElement(final ClassDescriptor<BEAN> classDescriptor, final Class<?> joinClass, final Integer nameSolverClassId, final String onLeftProperty, final String onRigthProperty) {
