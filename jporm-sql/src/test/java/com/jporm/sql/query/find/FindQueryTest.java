...
public class FindQueryTest extends BaseSqlTestApi {
    ...
    public void testQueryWithNullParameter() {
        Select select = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), Employee.class);

        select.selectFields(getBeanFields(Employee.class));
        select.where().eq("age", null); //$NON-NLS-1$
        System.out.println(select.renderSql());
        ...
        assertEquals(expectedSql , select.renderSql());
    ...
    public void testQuery2Count() {
        ...
        System.out.println(select.renderRowCountSql());
        ...
        assertEquals(expectedSql , select.renderRowCountSql());
    }

    @Test
    public void testQuery3() {
    	Select query = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), Employee.class);
...
//    	Select query = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), Employee.class, "employeeAlias");
//        query.selectFields(getBeanFields(Employee.class));
//
...
//        assertEquals(expectedSql , query.renderSql());
    ...
    public void testSubQuery1() {
    	...
    	Select subQuery1 = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), Employee.class);
        ...
        Select subQuery2 = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), People.class, "people");
        subQuery2.selectFields(getBeanFields(People.class));
        subQuery2.where().eq("people.firstname", "wizard"); //$NON-NLS-1$ //$NON-NLS-2$

        Select query = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), Employee.class, "e");
        query.selectFields(getBeanFields(Employee.class));
...
//                + "SELECT Employee_0.ID as hello, People_1.LASTNAME AS \"People.lastname\" FROM EMPLOYEE Employee_0 , PEOPLE People_1 WHERE Employee_0.ID >= People_1.ID ORDER BY People_1.LASTNAME ASC ) AND p_1.FIRSTNAME NOT IN ( SELECT people_0.FIRSTCLOB AS \"firstclob\", people_0.ID AS \"id\", people_0.SECONDBLOB AS \"secondblob\", people_0.BIRTHDATE AS \"birthdate\", people_0.LASTNAME AS \"lastname\", people_0.FIRSTNAME AS \"firstname\", people_0.FIRSTBLOB AS \"firstblob\", people_0.DEATHDATE AS \"deathdate\" FROM PEOPLE people_0 WHERE people_0.FIRSTNAME = ? ) "; //$NON-NLS-1$
    ...
    public void testSameTableJoinQuery1() {
        Select query = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), Employee.class, "e1");
        query.selectFields(getBeanFields(Employee.class));
    ...
    public void testCustomExpressionQuery() {
        Select query = new SelectImpl<>(new H2DBProfile(), getClassDescriptorMap(), new PropertiesFactory(), Employee.class, "Employee");
        query.selectFields(getBeanFields(Employee.class));
...
//        assertEquals(expectedSql , query.renderSql());
    ...
    private <BEAN> String[] getBeanFields(Class<BEAN> clazz) {
