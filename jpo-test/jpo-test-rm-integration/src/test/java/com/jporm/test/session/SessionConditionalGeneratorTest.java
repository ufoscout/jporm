    ...
    public void testFailSavingASavedBean() {
        ...
        try {
            jpOrm.tx().execute((_session) -> {
                ...
                assertEquals("people name 1", session.findById(People_ConditionalGenerator.class, 1).fetchOneUnique().getFirstname()); //$NON-NLS-1$
                try {
    ...
    public void testSavingBeanWithArbitraryId() {
        ...
        jpOrm.tx().execute((_session) -> {
            try {
                ...
                assertEquals("people name 1", session.findById(People_ConditionalGenerator.class, id).fetchOneUnique().getFirstname()); //$NON-NLS-1$
