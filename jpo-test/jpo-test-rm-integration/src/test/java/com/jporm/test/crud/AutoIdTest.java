...
public class AutoIdTest extends BaseTestAllDB {
	...
	public void testAutoId() {
		...
		AutoId autoIdLoad1 = jpOrm.transaction().execute((_session) -> {
			// LOAD
			AutoId autoIdLoad2 = _session.find(AutoId.class, autoId.getId() ).fetchUnique();
		...
		// LOAD
		final AutoId autoIdLoad2 = jpOrm.session().find(AutoId.class, autoId.getId() ).fetchUnique();
		assertNotNull(autoIdLoad2);
		...
		assertFalse(jpOrm.session().find(AutoId.class, autoId.getId() ).fetchOptional().isPresent());
	...
	public void testAutoIdInteger() {
		...
		// LOAD
		AutoIdInteger autoIdLoad1 = jpOrm.session().find(AutoIdInteger.class, autoId.getId() ).fetchUnique();
		...
		// LOAD
		final AutoIdInteger autoIdLoad3 = jpOrm.session().find(AutoIdInteger.class, autoId.getId() ).fetchUnique();
		...
		assertFalse(jpOrm.session().find(AutoIdInteger.class, autoId.getId() ).fetchOptional().isPresent());
