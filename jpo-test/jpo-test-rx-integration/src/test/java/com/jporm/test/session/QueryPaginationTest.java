...
public class QueryPaginationTest extends BaseTestAllDB {
    ...
    public void testMaxRowsPaginationWithOrderAsc() {
        transaction(session -> {
            ...
            return session.findQuery(CommonUser.class).maxRows(maxRows).where().ge("id", firstId).orderBy().asc("id").getList()
                    .thenApply(results -> {
    ...
    public void testMaxRowsPaginationWithOrderDesc() {
        ...
        transaction(session -> {
            int maxRows = new Random().nextInt(CommonUserQuantity) + 1;
            return session.findQuery(CommonUser.class).maxRows(maxRows).where().ge("id", firstId).orderBy().desc("id").getList()
                    .thenApply(results -> {
                        assertEquals(maxRows, results.size());
                        for (CommonUser commonUser : results) {
                            assertTrue(commonUser.getId() >= firstId);
    ...
    public void testFirstRowPaginationWithOrderAsc() {
        transaction(session -> {
            ...
            return session.findQuery(CommonUser.class).firstRow(firstRow).where().ge("id", firstId).orderBy().asc("id").getList()
                    .thenApply(results -> {
    ...
    public void testFirstRowPaginationWithOrderDesc() {
        transaction(session -> {
            int firstRow = new Random().nextInt(CommonUserQuantity);
            return session.findQuery(CommonUser.class).firstRow(firstRow).where().ge("id", firstId).orderBy().desc("id").getList()
                    .thenApply(results -> {
                        assertEquals(CommonUserQuantity - firstRow, results.size());
    ...
    public void testPaginationWithOrderAsc() {
        ...
        transaction(session -> {
            ...
            return session.findQuery(CommonUser.class).maxRows(maxRows).firstRow(firstRow).where().ge("id", firstId).orderBy().asc("id").getList()
                    .thenApply(results -> {
                        assertEquals(maxRows, results.size());
    ...
    public void testPaginationWithOrderDesc() {
        ...
        transaction(session -> {
            int firstRow = new Random().nextInt(CommonUserQuantity);
            int maxRows = new Random().nextInt(CommonUserQuantity - firstRow) + 1;
            return session.findQuery(CommonUser.class).maxRows(maxRows).firstRow(firstRow).where().ge("id", firstId).orderBy().desc("id").getList()
                    .thenApply(results -> {
                        assertEquals(maxRows, results.size());
