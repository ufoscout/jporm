...
public class PostgresqlConfig extends AbstractDBConfig {
	...
	public DBData getDBDataRxCore() {
		return buildDBData(DB_TYPE, "POSTGRESQL-RX-core", () -> getDataSource(DB_TYPE), (dataSource) -> new DataSourceConnectionProvider(new com.jporm.rm.session.datasource.DataSourceConnectionProvider(dataSource), 10));
	}

	@Lazy
	@Bean(name=DB_DATA_NAME + "-rx-vertx3")
	public DBData getDBDataRxVertx() {
		return buildDBData(DB_TYPE, "POSTGRESQL-RX-vertx3", () -> getDataSource(DB_TYPE), (dataSource) -> new Vertx3DataSourceConnectionProvider(new com.jporm.rm.session.datasource.DataSourceConnectionProvider(dataSource), Vertx.vertx()));
	}
