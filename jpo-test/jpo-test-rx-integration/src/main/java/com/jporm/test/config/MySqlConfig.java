...
public class MySqlConfig extends AbstractDBConfig {
	...
	public DBData getDBDataRxCore() {
		return buildDBData(DB_TYPE, "MYSQL-RX-core", () -> getDataSource(DB_TYPE), (dataSource) -> new ThreadPoolDataSourceConnectionProvider(dataSource, 10));
	}

	@Lazy
	@Bean(name=DB_DATA_NAME + "-rx-vertx3")
	public DBData getDBDataRxVertx() {
		return buildDBData(DB_TYPE, "MYSQL-RX-vertx3", () -> getDataSource(DB_TYPE), (dataSource) -> new Vertx3DataSourceConnectionProvider(dataSource, Vertx.vertx()));
	}
