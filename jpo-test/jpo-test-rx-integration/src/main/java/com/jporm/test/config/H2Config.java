...
public class H2Config extends AbstractDBConfig {
	...
	public DBData getDBDataRxCore() {
		return buildDBData(DB_TYPE, "H2-RX-core", () -> getDataSource(DB_TYPE), (dataSource) -> new DataSourceConnectionProvider(new com.jporm.rm.session.datasource.DataSourceConnectionProvider(dataSource), new ThreadPoolAsyncTaskExecutor(10)));
	}

	@Lazy
	@Bean(name=DB_DATA_NAME + "-rx-vertx3")
	public DBData getDBDataRxVertx() {
		return buildDBData(DB_TYPE, "H2-RX-vertx3", () -> getDataSource(DB_TYPE), (dataSource) -> new DataSourceConnectionProvider(new com.jporm.rm.session.datasource.DataSourceConnectionProvider(dataSource), new Vertx3AsyncTaskExecutor(Vertx.vertx())));
	}
