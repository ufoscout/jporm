...
public abstract class FieldDefaultNaming {
	 ...
	 * Es: dbName = GET_EMPLOYEE_AGE -> return employeeAge dbName = IS_MALE ->
	 * return isMale dbName = EMPLOYEE -> return employee
	 *
	 * @param dbName
	 * @return
	 */
	public static String getDBnameToJavanameDefaultMapping(final String dbName, final boolean startWithUpperCase) {
		...
		for (int i = 0; i < dbName.length(); i++) {
			final Character character = Character.valueOf(dbName.charAt(i));
			if (character == '_') {
				upperCase = true;
				continue;
			}
	...
	/**
	 * Return the default name of a getter for a property of type boolean. Es:
	...
	/**
	 * Return the default name of a getter for a property. Es: javaPropertyName
	 * = hello -> return getHello
	 *
	 * @param javaPropertyName
	 * @return
	 */
	...
	/**
	 * Remove the "is", "get" and "set" prefix from a method name and return the
	 * default column name associated to the method. Es: methodName =
	 * getEmployeeAge -> return EMPLOYEE_AGE methodName = isMale -> return MALE
	...
	public static String getDefaultMappingNameForMethod(final String javaName) {
		...
		final String[] prefixs = new String[] { Constants.GET_METHOD_PREFIX, Constants.SET_METHOD_PREFIX, Constants.IS_METHOD_PREFIX };
		boolean found = false;
	...
	/**
	 * Return the default name of a 'wither' for a property. Es:
	 * javaPropertyName = hello -> return withHello
	 *
	 * This is mostly used by @see
	 * <a href="https://immutables.github.io/">Immutables</a> library
	 *
	 * @param javaPropertyName
	 * @return
	 */
	public static String getDefaultWitherName(final String javaPropertyName) {
		return Constants.WITH_METHOD_PREFIX + javaPropertyName.substring(0, 1).toUpperCase() + javaPropertyName.substring(1);
	}

	/**
	 * Return the default database object name associated to the javaName Es:
	 * methodName = getEmployeeAge -> return GET_EMPLOYEE_AGE methodName =
	 * isMale -> return IS_MALE methodNAme = employee -> return EMPLOYEE
	 *
	 * @param methodName
	 * @return
	 */
