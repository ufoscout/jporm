...
//    	Select query = new SelectImpl<>(getClassDescriptorMap(), new PropertiesFactory(), Employee.class, "employeeAlias");
//        query.selectFields(getBeanFields(Employee.class));
//
...
//                + "SELECT Employee_0.ID as hello, People_1.LASTNAME AS \"People.lastname\" FROM EMPLOYEE Employee_0 , PEOPLE People_1 WHERE Employee_0.ID >= People_1.ID ORDER BY People_1.LASTNAME ASC ) AND p_1.FIRSTNAME NOT IN ( SELECT people_0.FIRSTCLOB AS \"firstclob\", people_0.ID AS \"id\", people_0.SECONDBLOB AS \"secondblob\", people_0.BIRTHDATE AS \"birthdate\", people_0.LASTNAME AS \"lastname\", people_0.FIRSTNAME AS \"firstname\", people_0.FIRSTBLOB AS \"firstblob\", people_0.DEATHDATE AS \"deathdate\" FROM PEOPLE people_0 WHERE people_0.FIRSTNAME = ? ) "; //$NON-NLS-1$
        ...
        assertTrue(query.renderSql(new H2DBProfile()).contains(" FROM EMPLOYEE e_0 INNER JOIN PEOPLE p_1 ON e_0.ID = p_1.FIRSTNAME NATURAL JOIN BLOBCLOB Blobclob_ByteArray_2"));
        ...
        assertTrue(query.renderSql(new H2DBProfile()).contains(" FROM EMPLOYEE Employee_0 , PEOPLE People_1 WHERE Employee_0.ID >= People_1.ID ORDER BY People_1.LASTNAME ASC ) AND p_1.FIRSTNAME NOT IN "));
    ...
    public void testSameTableJoinQueryThreeTimes() {
        ...
        assertTrue(query.renderSql(new H2DBProfile()).contains(" e1_0.SURNAME AS \"surname\""));
        assertTrue(query.renderSql(new H2DBProfile()).contains(" e1_0.EMPLOYEE_NUMBER AS \"employeeNumber\""));
        assertTrue(query.renderSql(new H2DBProfile()).contains(" FROM EMPLOYEE e1_0 INNER JOIN EMPLOYEE e2_1 ON e1_0.NAME = e2_1.NAME INNER JOIN EMPLOYEE e3_2 ON e1_0.SURNAME = e3_2.NAME "));
