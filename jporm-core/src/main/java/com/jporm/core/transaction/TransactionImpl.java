...
public class TransactionImpl<T> implements Transaction<T> {
	...
	public TransactionImpl(TransactionCallback<T> callback, final TransactionDefinition transactionDefinition, TransactionalSession session, SessionProvider sessionProvider) {
	...
	@Override
	public T now() {
		return sessionProvider.doInTransaction(session, transactionDefinition, callback);
	}
