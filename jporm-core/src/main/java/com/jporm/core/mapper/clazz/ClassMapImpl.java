...
import com.jporm.core.mapper.relation.ClassFKMapImpl;
...
public class ClassMapImpl<BEAN> implements ClassMap<BEAN> {
    ...
    @Override
    public <P> ClassFieldImpl<BEAN, P> getClassFieldByJavaName(final String javaName) {
        if (this.fieldClassMapByJavaName.containsKey(javaName)) {
            return (ClassFieldImpl<BEAN, P>) this.fieldClassMapByJavaName.get(javaName);
        }
        throw new OrmException("Property with name [" + javaName + "] not found in class " + this.mappedClass); //$NON-NLS-1$ //$NON-NLS-2$
    }

    public <P> void addClassField(final ClassFieldImpl<BEAN, P> classField) {
