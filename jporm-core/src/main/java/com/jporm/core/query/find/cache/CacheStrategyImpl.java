...
public class CacheStrategyImpl implements CacheStrategy {
    ...
    public <BEAN> void find(final String cacheName, final String sql, final List<Object> values,
            final List<String> ignoredFields, final OrmRowMapper<BEAN> srr,
        ...
        final Cache cache = serviceCatalog.getCacheManager().getCache(cacheName);
        ...
        List<BEAN> cached = (List<BEAN>) cache.get(key);
        if (cached!=null) {
            logger.debug("Sql results found in cache"); //$NON-NLS-1$
            for (int i = 0; i < cached.size(); i++) {
                srr.read(cached.get(i), i);
            }
        } else {
